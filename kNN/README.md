...
Se trata de un algoritmo [**basado en instancias**](https://en.wikipedia.org/wiki/Lazy_learning) (_lazy learning_). Esto significa que el algoritmo no aprende un modelo global o una generalización a partir del conjunto de entrenamiento, sino que compara cada nueva instancia con todas las instancias del conjunto de entrenamiento (guardadas en memoria). k-NN estima de forma local: la clase/etiqueta de cada nueva instancia depende únicamente de sus _k_ vecinos más cercanos (no se construye un modelo global). 
